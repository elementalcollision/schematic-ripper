...
> blonde piggyback era, ~1962–1964), since serviced — the filter caps and a few
> electrolytics have been replaced. Distinguishing 6G6-B from 6G6-C cannot be
> done from these nine frames alone; it needs a read of the transformer part-number
> stamps and the tube chart.**
...
- Small **modern Vishay 22 µF 50 V** electrolytics on cathode-bypass duty
  (IMG_0986, IMG_0987).
- These do not affect the circuit *identity*; they flag a recap, which is normal
  maintenance for a 60-year-old amp.
...
The two revisions are electrically near-identical. To close the gap, capture:
1. **Transformer stamps** — photograph the part-number/EIA date code on the power,
   output, and choke iron. `125P7A / 125C1A / 125A13A` would confirm 6G6-C.
2. **Tube chart / preamp tube layout** — confirm the 5881 × 2 / 7025 × 4 count
   directly rather than inferring from sockets.
3. **Bias circuit + a few coupling-cap values** — the residual electrical
   differences between B and C live here.
4. **Pot date codes** (EIA, e.g. `137-xxyy` CTS) — to pin the build year and
   cross-check against the 1962–64 window.
